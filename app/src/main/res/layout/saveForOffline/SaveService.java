...
public class SaveService extends Service {
	...
	private jonas.tool.saveForOffline.PageSaver pageSaver;
	private jonas.tool.saveForOffline.NotificationTools notificationTools;

	@Override
	public void onCreate() {
		executor = new ThreadPoolExecutor(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
		sharedPreferences = PreferenceManager.getDefaultSharedPreferences(jonas.tool.saveForOffline.SaveService.this);
		pageSaver = new jonas.tool.saveForOffline.PageSaver(new PageSaveEventCallback());
		notificationTools = new jonas.tool.saveForOffline.NotificationTools(this);
	}
	...
	private class PageSaveTask implements Runnable {
		...
		public void run() {
			try {
				...
				new jonas.tool.saveForOffline.Database(jonas.tool.saveForOffline.SaveService.this).addToDatabase(newSavedPageDirectory.getPath() + File.separator, pageSaver.getPageTitle(), pageUrl);

				if (sharedPreferences.getBoolean("generate_saved_page_thumbnails", true)) {
					Intent i = new Intent(jonas.tool.saveForOffline.SaveService.this, ScreenshotService.class);
					i.putExtra(jonas.tool.saveForOffline.Database.FILE_LOCATION, "file://" + newSavedPageDirectory.getPath() + File.separator + "index.html");
					i.putExtra(jonas.tool.saveForOffline.Database.ORIGINAL_URL, pageUrl);
					i.putExtra(jonas.tool.saveForOffline.Database.THUMBNAIL, newSavedPageDirectory + File.separator + "saveForOffline_thumbnail.png");
					startService(i);
				}
				...
				Toast.makeText(jonas.tool.saveForOffline.SaveService.this, "SaveService Exception: " + e.getMessage(), Toast.LENGTH_LONG);
				e.printStackTrace();
			}
		}
	...
	private class PageSaveEventCallback implements jonas.tool.saveForOffline.EventCallback {
