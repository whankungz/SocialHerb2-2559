...
public class ViewActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		title = incomingIntent.getStringExtra(jonas.tool.saveForOffline.Database.TITLE);
		fileLocation = incomingIntent.getStringExtra(jonas.tool.saveForOffline.Database.FILE_LOCATION);
		date = incomingIntent.getStringExtra(jonas.tool.saveForOffline.Database.TIMESTAMP);
		
		getActionBar().setDisplayHomeAsUpEnabled(true);
		getActionBar().setSubtitle(incomingIntent.getStringExtra(jonas.tool.saveForOffline.Database.TITLE));
...
//		webview.getSettings().setMediaPlaybackRequiresUserGesture(false);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case R.id.action_open_in_external:
				...
				Uri uri = Uri.parse(incomingIntent.getStringExtra(jonas.tool.saveForOffline.Database.ORIGINAL_URL));
			...
			case R.id.action_delete:
				...
				build = new AlertDialog.Builder(jonas.tool.saveForOffline.ViewActivity.this);
				...
				build.setPositiveButton("Delete",
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							SQLiteDatabase dataBase = new jonas.tool.saveForOffline.Database(jonas.tool.saveForOffline.ViewActivity.this).getWritableDatabase();
							Intent incomingIntent2 = getIntent();

							dataBase.delete(jonas.tool.saveForOffline.Database.TABLE_NAME, jonas.tool.saveForOffline.Database.ID + "=" + incomingIntent2.getStringExtra(jonas.tool.saveForOffline.Database.ID), null);

							String fileLocation = incomingIntent2.getStringExtra(jonas.tool.saveForOffline.Database.FILE_LOCATION);
							DirectoryHelper.deleteDirectory(new File(fileLocation).getParentFile());

							Toast.makeText(jonas.tool.saveForOffline.ViewActivity.this, "Saved page deleted", Toast.LENGTH_LONG).show();
	...
	private void showPropertiesDialog() {
		...
		t.setText("Saved from: \r\n" + incomingIntent.getStringExtra(jonas.tool.saveForOffline.Database.ORIGINAL_URL));
		...
		build.setNeutralButton("Copy file location to clipboard", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					Toast.makeText(jonas.tool.saveForOffline.ViewActivity.this, "File location copied to clipboard", Toast.LENGTH_SHORT).show();
	...
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == 5) {
			...
			Intent intent = new Intent(this, jonas.tool.saveForOffline.SaveService.class);
